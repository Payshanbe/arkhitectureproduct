...
export function MenuButton({ className, controlsId, isOpen, onClick }: MenuButtonProps) {
  return (
    <button
      ...
      className={cn(
        "inline-flex min-h-11 min-w-11 items-center justify-center rounded-none text-[length:var(--font-size-ui)] uppercase leading-[var(--line-height-ui)] tracking-[var(--letter-spacing-ui)] transition-colors duration-base ease-architectural-out",
        className,
      )}
      type="button"
      onClick={onClick}
