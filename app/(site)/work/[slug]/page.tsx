...
function isMedia(value: Project["coverImage"] | NonNullable<Project["seo"]>["image"]): value is Media {
...
function normalizeLocation(project: Project) {
  const location = [project.city ?? project.location, project.country].filter(Boolean).join(", ");

  return location || undefined;
}

function getCategoryTitle(project: Project) {
  return isProjectCategory(project.category) ? project.category.title : undefined;
}
...
async function getPublishedProject(slug: string): Promise<Project | null> {
  try {
...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  ...
  const canonical = absoluteUrl(project.seo?.canonical ?? `/work/${project.slug}`);
